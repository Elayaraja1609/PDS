...
namespace API.Controllers
{
	public class DeliveryController(IDeliveryService _deliveryService) : BaseApiController
	{
		...
		//}
		[HttpGet("All")]
		public async Task<ActionResult<IList<DeliveryDetail>>> Get()
		{
			var deliveries = await _deliveryService.GetAll();
			return Ok(deliveries);
		}
		[HttpGet("GetDeliveryById/{id}")]
		public async Task<ActionResult<DeliveryDetail>> GetDeliveryById(int id)
		{
		...
		[HttpPost]
		public async Task<ActionResult<DeliveryDetail>> Create([FromBody] DeliveryDetailsDto delivery)
		{
			var created = await _deliveryService.CreateDeliveryAsync();
			return CreatedAtAction(nameof(GetById), new { id = created.DeliveryId }, created);
		}
		[HttpGet("{id}")]
		public async Task<ActionResult<DeliveryDetail>> GetById(int id)
		{
